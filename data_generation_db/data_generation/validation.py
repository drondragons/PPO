...
class Validator:
 ...
 должно содержать максимум {option[1]!r} допустимых символов!\n')
    ...
    @staticmethod
    def interval_validator(name, value, *option):
        if option[0] and option[0] > value:
            raise ValueError(f'\nЗначение атрибута {name!r}={value!r}\
 должно быть больше {option[0]!r}!\n')

        if option[1] and option[1] < value:
            raise ValueError(f'\nЗначение атрибута {name!r}={value!r}\
 должно быть меньше {option[1]!r}!\n')
